...
However, when using AnyEvent::Task, libraries that you use in the client must be L<AnyEvent> compatible. This restriction obviously does not apply to your server code. That is the main purpose of this module: accessing blocking resources from an asynchronous program.
...
In order to handle exceptions in a meaningful way with this module, you must use L<Callback::Frame>.

There are about a million CPAN modules that do similar things.
