...
The synopses make this module look much more complicated than it actually is. In a nutshell, blocking worker processes are forked off by a server when a client needs one. The client keeps as many of these workers around as it wants and delegates tasks to them asynchronously. 
...
Note that the clients don't implement production-grade error checking (see the L<ERROR HANDLING> section).
...
After both the server and client are initialised, each process must enter AnyEvent's "main loop" in some way, possibly just C<< AE::cv->recv >>. The C<run> method on the server object is a convenient short-cut for this.

To checkout a worker process you call the C<checkout> method on the client object. The C<checkout> method doesn't require any arguments, but several optional ones such as C<timeout> are described below. As long as the checkout object is around, this checkout has exclusive access to the worker.
...
Often you will want to start the client and server as completely separate processes as shown in the synopses.
...
Since the C<fork_task_server> constructor forks and requires using AnyEvent in both the parent and child processes, it is important that you not install any AnyEvent watchers before calling it. The usual caveats about forking AnyEvent applications apply (see the L<AnyEvent> docs).
...
Because there isn't any way for the callback to indicate the context it desires, the interface sub is always called in scalar context.
...
However, when using AnyEvent::Task, libraries that you use in the client must be L<AnyEvent> compatible. This restriction obviously does not apply to your server code. That is the main purpose of this module: accessing blocking resources from an asynchronous program. In your server code, when there is an error condition you should simply C<die> as in a synchronous program.
...
However, once the checkout object is destroyed, by default the worker will be shutdown instead of returning to the client's worker pool as in the normal case where no errors were thrown. This is a "safe-by-default" behaviour that may help in the event that an exception thrown by a worker leaves the worker process in a broken/inconsistent state for some reason. This can be overridden by setting the C<dont_refork_after_error> option to C<1> in the client constructor. This will only matter if errors are being thrown frequently and your C<setup> routines take a long time (aside from the setup routine, creating new workers is quite fast since the server has already compiled all the application code and just has to fork).

There are cases where workers will never be returned to the worker pool: workers that have thrown fatal errors such as loss of connection or hung worker timeout errors. These errors are stored in the checkout and for as long as the checkout exists, any methods on the checkout will immediately return the stored fatal error. Your client process can invoke this behaviour manually by calling the C<throw_fatal_error> method on a checkout object to cancel an operation and force terminate a worker.



=head2 Rationale for Callback::Frame

Why not just call the callback but set C<$@> to indicate an error has occurred? This is the approach taken with L<AnyEvent::DBI> for example. I believe the L<Callback::Frame> interface is superior to this method. In a synchronous program, exceptions are out-of-band messages and code doesn't need to locally handle them. It can let them "bubble up" the stack, perhaps to a top-level error handler. Invoking the callback when an error occurs forces exceptions to be handled in-band.

Why not just have AnyEvent::Task expose an error callback? This is the approach taken by L<AnyEvent::Handle> for example. I believe Callback::Frame is superior to this method also. Although separate callbacks are (sort of) out-of-band, you still have to write error handler callbacks and do something relevant locally instead of having a single "catch-all" top-level error handler.
...
In servers, Callback::Frame helps you maintain the "dynamic state" (error handlers and dynamic variables) installed for a single connection. In other words, any errors that occur while servicing that connection will be able to be caught by an error handler specific to that connection. This lets you send error responses and collect associated log messages in a Log::Defer object specific to that connection.

Callback::Frame is designed to be easily used with callback-based libraries that don't know about Callback::Frame. C<fub> is a shortcut for C<frame> with just the C<code> argument. Instead of passing C<sub { ... }> into libraries you can pass in C<fub { ... }>. When invoked, this wrapped callback will first re-establish any error handlers that you installed with C<frame> and then run your actual callback code. In-band error signalling should be handled with callbacks such as C<fub { die $@ if $@; ... }>. Separate error callbacks should be simply C<fub { die "failed becase ..." }>.

It's important that all callbacks to all libraries be created with C<fub> (or C<frame>) even if you don't expect them to fail so that the dynamic context is preserved for nested callbacks that might. An exception is the callbacks provided to AnyEvent::Task checkouts: These are automatically wrapped in frames for you (although passing in fubs won't hurt anything).

The L<Callback::Frame> documentation explains all of this in much more detail.
...
The current AnyEvent::Task server obeys a very specific implementation policy: It is like a CGI servers in that each process it forks is guaranteed to be handling only one connection at once so it can perform blocking operations without worrying about holding up other connections.
...
Client processes can be started and checkouts can be obtained before the server is even started. The client will continue to try to connect to the server to obtain worker processes until either the server starts or the checkout's timeout period lapses. As well as freeing you from having to start your services in the "right" order, this also means servers can be restarted without throwing any errors (sometimes called "zero-downtime restarts").

The client even decides how many minimum workers should be in the pool upon start-up and how many maximum workers it requires before checkout creation requests are queued instead of forking more workers. The server is really just a dumb fork-on-demand server and most of the sophistication is in the asynchronous client.
...
Although this module's interface is now stable, there are few remaining TODO items (see the bottom of Task.pm).
...
Optionally limit number of times a worker can be checked out before reforking a new one to deal with leaky code
